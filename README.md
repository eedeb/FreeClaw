...
- **Multi-provider fallback** — runs on Google (Gemini) by default; if a call fails, it automatically retries on Cerebras or OpenRouter if you've configured a key for either
...
Google (Gemini) API call ← trimmed message history + tools
    │  (falls back to Cerebras or OpenRouter on failure)
    │
